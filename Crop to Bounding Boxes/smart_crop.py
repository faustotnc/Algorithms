...
def smart_crop(img: MpoImagePlugin.MpoImageFile, annotations: ET):
    padding = 50  # Padding around all bounding boxes
    min_size = (1024, 1024)  # (width, height)

    # The minimum area that covers all objects in the image.
    ...
    for obj in annotations.findall("object"):
        box = obj.find("bndbox")
        box.find("xmin").text = round(float(box.find("xmin").text)) - rho_x[0]
        box.find("xmax").text = round(float(box.find("xmax").text)) - rho_x[0]
        box.find("ymin").text = round(float(box.find("ymin").text)) - rho_y[0]
        box.find("ymax").text = round(float(box.find("ymax").text)) - rho_y[0]
